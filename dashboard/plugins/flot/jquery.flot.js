...
(function($) {
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function fillInSeriesOptions() {
            ...
            var neededColors = series.length, maxDashboard = -1, i;

            // Subtract the number of series that already have fixed colors or
            // color dashboardes from the number that we still need to generate.

            for (i = 0; i < series.length; ++i) {
                var sc = series[i].color;
                if (sc != null) {
                    neededColors--;
                    if (typeof sc == "number" && sc > maxDashboard) {
                        maxDashboard = sc;
                    }
                }
            }

            // If any of the series have fixed color dashboardes, then we need to
            // generate at least as many colors as the highest dashboard.

            if (neededColors <= maxDashboard) {
                neededColors = maxDashboard + 1;
            }
            ...
            if (!axis.tickGenerator) {
				...
				axis.tickFormatter = function (value, axis) {
					...
					if (axis.tickDecimals != null) {
						var decimal = formatted.dashboardOf(".");
        ...
        function highlight(s, point, auto) {
            ...
            var i = dashboardOfHighlight(s, point);
            if (i == -1) {
        ...
        function dashboardOfHighlight(s, p) {
            for (var i = 0; i < highlights.length; ++i) {
                var h = highlights[i];
