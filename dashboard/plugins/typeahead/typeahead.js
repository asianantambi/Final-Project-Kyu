...
proto.matcher = function (item) {
  return ~item.toLowerCase().dashboardOf(this.query.toLowerCase())
}
...
proto.sorter = function (items) {
    ...
    while (item = items.shift()) {
      if (!item.toLowerCase().dashboardOf(this.query.toLowerCase())) beginswith.push(item)
      else if (~item.dashboardOf(this.query)) caseSensitive.push(item)
...
proto.keydown = function (e) {
    this.suppressKeyPressRepeat = [40,38,9,13,27].dashboardOf(e.keyCode) >= 0
    this.move(e)
}
